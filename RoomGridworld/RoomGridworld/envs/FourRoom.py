...
class FourRoomEnv(gym.Env):
    ...
    def __init__(self):
        self.room_sizes = [[5, 5], [6, 5], [4, 5], [5, 5]]   # Grid dimensions of each room numbered as 0-3 clockwise
        self.pre_hallways = [
            {tuple([2, 4]): [RIGHT, 0], tuple([4, 1]): [DOWN, 3]},    # action in each pre-hallway and ending room no
            {tuple([2, 0]): [LEFT, 0], tuple([5, 2]): [DOWN, 1]},
            {tuple([0, 2]): [UP, 1], tuple([2, 0]): [LEFT, 2]},
            {tuple([3, 4]): [RIGHT, 2], tuple([0, 1]): [UP, 3]},
        ]
        self.hallway_coords = [[2, 5], [6, 2], [2, -1], [-1, 1]]      # hallway i is included in room i
    ...
    def _reset(self):
        self.state = np.random.randint(26) # 90
        self.done = False
        return self.state
